function [output] = Preproc_HCPData_PostMinPreproc_TaskRest_FIR(SUBJECTLIST)
...
ANALYSISNAME='TaskFCMethods_HCPData_FIR';
mfilenameRep='Preproc_PostMinPreproc_TaskAndRest_FIR';
...
HIGHPASS_HZ=0.008;
%LOWPASS_HZ=0.09;
LOWPASS_HZ=0.2; %Selecting 0.2 Hz because primary frequency of HRF, unlikely much signal above this possible with fMRI
...
for subjIndex=1:numSubjs
    ...
    if and(~isempty(TASKRUNS), or(RERUN_TASKREGRESSORPREP == 1, ~exist(savedfile, 'file')))
        ...
        disp('Obtaining regressors for each task event, from original EV files');

        rawDataDir='/projects/ExternalDatasets/HCPData/data/minimalpreproc/';
        ...
        taskRunNames=RUNNAMES(TASKRUNS);
        
        %Create data structure for event info: event onset, event duration, run number, event label
        eventInfo.onsets=[];
        eventInfo.durations=[];
        eventInfo.runnum=[];
        ...
        eventCount=1;
        for runNum = 1:length(taskRunNames)
            %Go through each timing file, pulling in onsets and durations
            for timingFileNum=1:length(timingFilenamesByRun{runNum})
                filename=[timingFilenamesByRun{runNum}{timingFileNum} '.txt'];
                fid=fopen([rawDataDir num2str(subjNum) '/MNINonLinear/Results/' taskRunNames{runNum} '/EVs/' filename]);
                ...
                fclose(fid);
                numEvents=size(timingInfo,2);
                for thisEventNum=1:numEvents
                    %Saving onset
                    eventInfo.onsets(eventCount)=timingInfo(1,thisEventNum);
                    %Saving duration
                    eventInfo.durations(eventCount)=timingInfo(2,thisEventNum);
                    %Saving run number
                    eventInfo.runnum(eventCount)=runNum;
                    %Saving event label
                    eventInfo.eventlabel{eventCount}=timingFilenamesByRun{runNum}{timingFileNum};
                    eventCount=eventCount+1;
                end
            end
        end
        
        condLabelsInOrder={'fear' 'neut' 'win' 'loss' 'story' 'math' 'cue' 'lf' 'rf' 'lh' 'rh' 't' 'relation' 'match' 'mental' 'rnd' '0bk_body' '0bk_faces' '0bk_places' '0bk_tools' '2bk_body' '2bk_faces' '2bk_places' '2bk_tools'};
        %Remove the motor cue events (otherwise FIR model becomes rank deficient due to event overlaps)
        %condLabelsInOrder={'fear' 'neut' 'win' 'loss' 'story' 'math' 'lf' 'rf' 'lh' 'rh' 't' 'relation' 'match' 'mental' 'rnd' '0bk_body' '0bk_faces' '0bk_places' '0bk_tools' '2bk_body' '2bk_faces' '2bk_places' '2bk_tools'};
        %condLabelsInOrder={'0bk_body' '0bk_faces' '0bk_places' '0bk_tools' '2bk_body' '2bk_faces' '2bk_places' '2bk_tools'};
        
        %Create FIR design matrix; go through event-wise
        %extraTRsPerEvent=0;
        %extraTRsPerEvent=18;
        extraTRsPerEvent=25;    %18 seconds; 18/.720=25
        %extraTRsPerEvent=22;    %16 seconds; 16/.720=22
        taskRunDurs=RUNLENGTHS(TASKRUNS);
        designMatrix_FIR=zeros(sum(taskRunDurs),length(condLabelsInOrder)*10);
        columnCountTotal=1;
        for condNum = 1:length(condLabelsInOrder)
            
            thisCondLabel=condLabelsInOrder(condNum);
            eventNumsForCond=find(strcmp(eventInfo.eventlabel,thisCondLabel));
            
            %Calculate timing of each event (all runs merged previously)
            overallOnsetsInTRs=zeros(length(eventNumsForCond),1);
            dursInTRs=zeros(length(eventNumsForCond),1);
            for thisEventCount = 1:length(eventNumsForCond)
                
                thisEventNum=eventNumsForCond(thisEventCount);
                
                %Calculate number of TRs for prior runs
                eventRunNum=eventInfo.runnum(thisEventNum);
                priorRunTRCount=sum(taskRunDurs(1:(eventRunNum-1)));
                
                %Calculate within-run timing (in TRs)
                onsetInSec=eventInfo.onsets(thisEventNum);
                onsetInTRs=floor(onsetInSec/TR_INSECONDS);
                
                %Calculate overall timing
                overallOnsetsInTRs(thisEventCount)=priorRunTRCount+onsetInTRs;
                
                %Calculation duration in TRs
                dursInTRs(thisEventCount)=ceil(eventInfo.durations(thisEventNum)/TR_INSECONDS);
            ...
            for thisEventCount = 1:length(eventNumsForCond)
                ...
                thisEventNum=eventNumsForCond(thisEventCount);
                thisEventRun=eventInfo.runnum(thisEventNum);
                thisRunEndTime=sum(taskRunDurs(1:(thisEventRun-1)))+taskRunDurs(thisEventRun);
                
                thisTimePoint=overallOnsetsInTRs(thisEventCount);
                for durCount = 1:(dursInTRs(thisEventCount) + extraTRsPerEvent)
                    if thisTimePoint <= thisRunEndTime  %Make sure not going past end of run with regressor
                        designMatrix_FIR(thisTimePoint,columnCountTotal+durCount-1)=1;
                    else
                        flagEventCutoff=1;
                    end
            ...
            maxDurForCond=max(ceil(eventInfo.durations(eventNumsForCond)/TR_INSECONDS));
            minDurForCond=min(ceil(eventInfo.durations(eventNumsForCond)/TR_INSECONDS));
            
            if maxDurForCond ~= minDurForCond
                disp(['WARNING: Durations of events are not equal across instances for condition: ' condLabelsInOrder{condNum}])
            end
            if flagEventCutoff
                disp(['WARNING: Durations of events are not equal across instances for condition (due to end of run cutting off event time points): ' condLabelsInOrder{condNum}])
            end
            
            columnCountTotal=columnCountTotal+maxDurForCond+extraTRsPerEvent;
        ...
        %Remove motor task encoding events (otherwise FIR regressor matrix becomes rank deficient)
        %regressorMatrix=regressorMatrix(:,[1:6 8:24]);
        %NUMCONDS=23;
        
        %Check example of timing alignment. Flip back-and-forth between plots to ensure alignment to original block regressors.
        checktiming=0;
        if checktiming==1
            %Task 7 regressors
            figure;imagesc(designMatrix_FIR(3000:end,780:end))
            figure;imagesc(regressorMatrix(3000:end,16:end))
            %Task6 regressors
            %figure;imagesc(designMatrix_FIR(2500:3000,304:370))
            %figure;imagesc(regressorMatrix(2500:3000,14:15))
            %Task 5
            %figure;imagesc(designMatrix_FIR(2000:2500,260:304))
            %figure;imagesc(regressorMatrix(2000:2500,12:13))
            %Task 4
            %figure;imagesc(designMatrix_FIR(1400:2100,169:260))
            %figure;imagesc(regressorMatrix(1400:2100,7:11))
            %Task 3 -PROBLEM? (fixed)
            %figure;imagesc(designMatrix_FIR(800:1500,130:190))
            %figure;imagesc(regressorMatrix(800:1500,5:6))
            %Task 2
            %figure;imagesc(designMatrix_FIR(350:900,40:120))
            %figure;imagesc(regressorMatrix(350:900,3:4))
            %Task 1 - PROBLEM? (fixed)
            %figure;imagesc(designMatrix_FIR(1:350,1:55))
            %figure;imagesc(regressorMatrix(1:350,1:2))
        end
        
        taskTiming.taskdesignmat=designMatrix_FIR;
        %taskTiming.taskdesignmat=regressorMatrixFIR;
        ...
        %Save to CSV file for separate analysis
        csvwrite(designMatrix_FIR, [timingfileDir '\HCP_FIRtaskregressionmatrix_ ' num2str(subjNum) '.csv']);
    ...
    if and(~isempty(TASKRUNS), or(RERUN_TASKGLM == 1, ~exist(savedfile, 'file')))
        ...
        taskGLMVars = runGLM(tseriesMatSubj, NUMREGRESSORS_NUISANCE, nuisanceTSVars, taskTiming.taskdesignmat, RUNLENGTHS, TASKRUNS, NPROC, NUMPARCELS, GSR, visualizeDesignMatrix);
...
%for (regionNum=1:NUMPARCELS)
    ...
    %Removing singular matrix warning
